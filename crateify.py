# This script takes the stm32g0xx runtimes generated by build-rts.py and
...
#  - adds stm32g0xx_runtime_config.ads which renames the crate config package
...
def patch_target_options(gpr_file: pathlib.Path, profile: str, target: str):
    ...
    E.g. for light-tasking-stm32g0xx:
    BUILD becomes LIGHT_TASKING_STM32G0XX_BUILD
    LIBRARY_TYPE becomes LIGHT_TASKING_STM32G0XX_LIBRARY_TYPE
...
def main():
    ...
    gen_from_template(
        template_file=templates_dir / "stm32g0xx_runtime_config.ads.in",
        out_file=runtime_dir / "gnat_user" / "stm32g0xx_runtime_config.ads",
        template_values=template_values,
    )

    shutil.copytree(
        src=pathlib.Path(__file__).parent / "stm32g0_src" / "ld",
        dst=runtime_dir / "ld",
        dirs_exist_ok=True,
    )
